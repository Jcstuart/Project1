...
class Event { 
    ...
    function create_record() { // display "create" form
        ...
        $this->generate_form_group("event_date", $this->dateError, $this->date, "autofocus");
        $this->generate_form_group("event_time", $this->timeError, $this->time);
        $this->generate_form_group("event_location", $this->locationError, $this->location);
		$this->generate_form_group("event_description",$this->descriptionError, $this->description);
    ...
    function read_record($id) { // display "read" form
        ...
        $this->generate_form_group("event_date", $this->dateError, $this->date, "disabled");
        $this->generate_form_group("event_time", $this->timeError, $this->time, "disabled");
        $this->generate_form_group("event_location", $this->locationError, $this->location, "disabled");
        $this->generate_form_group("event_description", $this->descriptionError, $this->description, "disabled");
    ...
    function update_record($id) { // display "update" form
        ...
        $this->generate_form_group("event_date", $this->dateError, $this->date, "autofocus onfocus='this.select()'");
        $this->generate_form_group("event_time", $this->timeError, $this->time);
        $this->generate_form_group("event_location", $this->locationError, $this->location);
		$this->generate_form_group("event_description",$this->descriptionError, $this->description);
    ...
    function insert_db_record () {
        if ($this->fieldsAllValid ()) { // validate user input
            ...
            $sql = "INSERT INTO $this->tableName (date,time,location,description) values(?, ?, ?, ?)";
            $q = $pdo->prepare($sql);
            $q->execute(array($this->date,$this->time,$this->location,$this->description));
            Database::disconnect();
            header("Location: events.php"); // go back to "list"
        }
        else {
    ...
    function update_db_record ($id) {
        ...
        if ($this->fieldsAllValid()) {
            ...
            $pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "UPDATE $this->tableName  set date = ?, time = ?, location = ?, description = ? WHERE id = ?";
            $q = $pdo->prepare($sql);
            ...
            <body>
                <div class='container'>
                    <div class='span10 offset1'>
                        ...
                        <form class='form-horizontal' action='events.php?fun=$funNext' method='post'>                        
                    ";
    ...
    function list_records() {
        ...
        echo "
            ...
            <body>
                <a href='https://github.com/Jcstuart/Project1.git' target='_blank'>Github</a><br />
                <div class='container'>
                    <p class='row'>
                        <h3>$this->title" . "s" . "</h3>
                    </p>
                    <p>
                        <a href='events.php?fun=display_create_form' class='btn btn-success'>Create</a>
                    </p>
                    <div class='row'>
        ...
        foreach ($pdo->query($sql) as $row) {
            ...
            echo "<td width=250>";
            echo "<a class='btn btn-info' href='events.php?fun=display_read_form&id=".$row["id"]."'>Read</a>";
            echo "&nbsp;";
            echo "<a class='btn btn-warning' href='events.php?fun=display_update_form&id=".$row["id"]."'>Update</a>";
            echo "&nbsp;";
            echo "<a class='btn btn-danger' href='events.php?fun=display_delete_form&id=".$row["id"]."'>Delete</a>";
            echo "</td>";
            echo "</tr>";
        }
        Database::disconnect();        
